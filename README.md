...
| <a name="input_hosts_definition"></a> [hosts\_definition](#input\_hosts\_definition) | List of PostgreSQL hosts. | <pre>list(object({<br>    name                    = optional(string, null)<br>    zone                    = string<br>    subnet_id               = optional(string, null)<br>    assign_public_ip        = optional(bool, false)<br>    replication_source_name = optional(string, null)<br>    priority                = optional(number, null)<br>  }))</pre> | `[]` | no |
| <a name="input_labels"></a> [labels](#input\_labels) | Set of label pairs to assing to the PostgreSQL cluster | `map(any)` | `{}` | no |
